...
describe('injectors', () => {
  ...
  describe('ejectSaga helper', () => {
    beforeEach(() => {
      ...
      injectSaga = injectSagaFactory(reduxStore, true);
      ejectSaga = ejectSagaFactory(reduxStore, true);
    });
    ...
    it('should cancel a saga in a default mode', () => {
      ...
      reduxStore.injectedSagas.test = { task: { cancel } };
      ejectSaga('test');
    ...
    it('should not cancel a daemon saga', () => {
      const cancel = jest.fn();
      reduxStore.injectedSagas.test = { task: { cancel }, mode: DAEMON };
      ejectSaga('test');
    ...
    it("should remove non daemon saga's descriptor in production", () => {
      ...
      expect(reduxStore.injectedSagas.test).toBe('done');
      expect(reduxStore.injectedSagas.test1).toBe('done');
    ...
    it("should not remove daemon saga's descriptor in production", () => {
      ...
      expect(reduxStore.injectedSagas.test.saga).toBe(testSaga);
      process.env.NODE_ENV = originalNodeEnv;
    });
  ...
  describe('injectSaga helper', () => {
    ...
    it('should pass args to saga.run', () => {
      const args = {};
      reduxStore.runSaga = jest.fn();
      injectSaga('test', { saga: testSaga }, args);

      expect(reduxStore.runSaga).toHaveBeenCalledWith(testSaga, args);
    });
    ...
    it('should not start daemon and once-till-unmount sagas if were started before', () => {
      ...
      expect(reduxStore.runSaga).toHaveBeenCalledTimes(2);
    });
    ...
    it('should start any saga that was not started before', () => {
      ...
      expect(reduxStore.runSaga).toHaveBeenCalledTimes(3);
    });

    it('should restart a saga if different implementation for hot reloading', () => {
      const cancel = jest.fn();
      reduxStore.injectedSagas.test = { saga: testSaga, task: { cancel } };
      reduxStore.runSaga = jest.fn();
      ...
      expect(reduxStore.runSaga).toHaveBeenCalledWith(testSaga1, undefined);
    });

    it('should not cancel saga if different implementation in production', () => {
      process.env.NODE_ENV = 'production';
      const cancel = jest.fn();
      reduxStore.injectedSagas.test = {
        saga: testSaga,
        task: { cancel },
    ...
    it('should save an entire descriptor in the saga registry', () => {
      ...
      expect(reduxStore.injectedSagas.test.foo).toBe('bar');
    });
  });
});
